...
function phasesFrom(spec: SpecDoc | undefined): Phase[] {
  const raw = Array.isArray(spec?.phases) ? (spec.phases as Record<string, unknown>[]) : [];
  if (raw.length === 0) {
    return [{ title: "", description: "", acceptance: "", touches: "" }];
  }
  return raw.map((p) => ({
    title: typeof p.title === "string" ? p.title : "",
    description: typeof p.description === "string" ? p.description : "",
    acceptance: Array.isArray(p.acceptance) ? (p.acceptance as string[]).join("\n") : "",
    touches: Array.isArray(p.touches) ? (p.touches as string[]).join("\n") : "",
  }));
}

function str(spec: SpecDoc | undefined, key: string, fallback = ""): string {
  const value = spec?.[key];
  return typeof value === "string" ? value : fallback;
}

function list(spec: SpecDoc | undefined, key: string, fallback = ""): string {
  const value = spec?.[key];
  return Array.isArray(value) ? (value as string[]).join(", ") : fallback;
}

function num(spec: SpecDoc | undefined, key: string, fallback: number): number {
  const value = spec?.[key];
